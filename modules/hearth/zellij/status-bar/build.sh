#!/bin/bash
# Rebuilds the zellij-status-bar WASM plugin (nix cross-build, no rustup
...
echo "Building zellij-status-bar WASM plugin (pkgsCross.wasi32)..."
...
cp -f result/bin/zellij-status-bar.wasm ../plugins/zellij_status_bar.wasm
rm result
