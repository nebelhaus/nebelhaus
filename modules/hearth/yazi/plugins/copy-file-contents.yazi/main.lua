...
-- Vendored copy of AnirudhG07/plugins-yazi//copy-file-contents.yazi. Not yet in
-- nixpkgs' `yaziPlugins`, so we ship it alongside the tree like the glow vendor.
-- Copies the contents (not the path) of the hovered or selected file(s) into the
-- system clipboard. Uses only the modern (26.x) Lua API: ya.sync / ya.clipboard /
-- ya.notify. Drop this vendor once upstream lands in nixpkgs.
