...
SIBLING="/tmp/sketchybar_navigate_state"  # navigate_mode.sh's state — `on`
                                          # clears it so a navigate→resize hop
                                          # is ONE script call (two async calls
                                          # would race on the same pill)
